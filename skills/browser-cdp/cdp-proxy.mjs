...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    // GET /new?url=xxx 或 POST /new (body JSON { url }) - 创建新后台 tab
    else if (pathname === '/new') {
      let targetUrl = q.url || 'about:blank';
      if (req.method === 'POST' && !q.url) {
        try {
    ...
    // GET /navigate?target=xxx&url=yyy 或 POST /navigate (body JSON { url }) - 导航（自动等待加载）
      ...
      if (!targetUrl) {
        res.statusCode = 400;
        res.end(JSON.stringify({ error: '需要 url 参数' }));
        return;
      }
